...
def load(path: Path):
    # utf-8-sig safely strips a possible BOM from the first field name.
    with path.open(newline="", encoding="utf-8-sig") as f:
        return list(csv.DictReader(f))


def clean_id(value: str | None) -> str:
    """Normalize identifiers without changing their semantic content."""
...
source_ids = {clean_id(r["source_id"]) for r in sources}
assert source_ids == {"PAT_US5599895A"}, source_ids

for group_name, group_rows in [("formulations", forms), ("materials", mats), ("contrasts", contrasts)]:
    for row in group_rows:
        assert clean_id(row["source_id"]) in source_ids, (group_name, row)

assert len(forms) == 17
assert len({clean_id(r["record_id"]) for r in forms}) == len(forms)
...
for row in forms:
    ...
    assert abs(total - reported) < 1e-6, (clean_id(row["record_id"]), total, reported)
    # Patent tables contain rounding to one or two decimals; accept at most 0.11 wt% closure error.
    assert abs(reported - 100.0) <= 0.11, (clean_id(row["record_id"]), reported)
...
form_ids = {clean_id(r["record_id"]) for r in forms}
assert len(contrasts) == 3
for row in contrasts:
    assert clean_id(row["baseline_record_id"]) in form_ids
    assert clean_id(row["variant_record_id"]) in form_ids
...
material_ids = {clean_id(r["material_id"]) for r in mats}
required = {
...
assert not missing, f"missing material IDs: {sorted(missing)}; have: {sorted(material_ids)}"
